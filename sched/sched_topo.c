 ...
 * Let
 *     L = least, M = most,  A = available, P = pod, S = switch,
 *     N = # of nodes in a job,
 ...
 * allocations and reservations of each type (T1, T2, T3). Only for testing.
 */
                            ...
                            bool below_match)
{
     ...
     */
    bool matched_type = resrc_match_resource (resrc, resrc_reqst, false, &reason);
...
int process_args (flux_t *h, char *argz, size_t argz_len, const sched_params_t *sp)
{
    ...
    for (entry = argz;
        ...
        } else if (!strncmp ("verbose", entry, sizeof ("verbose"))) {
            verbose = true;
        } else {
...
static int allocate_topo_structures (flux_t *h, resrc_tree_t *root)
{
    ...
    if (verbose) {
        flux_log (h, LOG_DEBUG, "Tree depth = %d\n", tree_depth);
    }
    ...
    for (d = 0; d < tree_depth; d++) {
        ...
        if (verbose) {
            flux_log (h, LOG_DEBUG,
                "Tree fan-out at level %d = %"PRId64", total sz = %"PRId64"\n",
                d, levels[d], t_d);
        }
    }
 ...
 * TODO: Finalize never gets called */
void finalize ()
{
    free (levels);
