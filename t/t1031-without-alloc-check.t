...
test_expect_success 'submit a node-exclusive job that exceeds its time limit' '
	JOBID=$(flux submit -N1 -x -t2s sleep 30)
'
test_expect_success 'fail to match while the previous epilog is still running' '
	flux run --dry-run -N1 -x -t1s sleep 2 >jobspec.json &&
	flux job wait-event --timeout=5s $JOBID epilog-start &&
