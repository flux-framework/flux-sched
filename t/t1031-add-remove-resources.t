...
if test_have_prereq ASAN; then
    skip_all='skipping add/remove resource tests under AddressSanitizer'
    test_done
fi
...
test_expect_success 'add-subgraph fails on empty input' '
	test_must_fail flux ion-resource add-subgraph ""
'

test_expect_success 'add-subgraph fails on subgraph with different root' '
	test_must_fail flux ion-resource add-subgraph \
		${SHARNESS_TEST_SRCDIR}/data/resource/jgfs/issue1260.json
'
...
test_expect_success 'remove-subgraph fails on empty input' '
	test_must_fail flux ion-resource remove-subgraph ""
'

test_expect_success 'remove-subgraph fails on malformed input' '
	test_must_fail flux ion-resource remove-subgraph /cluster0
'
