...
# run api doc
def run_apidoc(app):
    # Move import inside so this file can be exec'd without sphinx installed
    # pylint: disable=import-outside-toplevel
    from sphinx.ext.apidoc import main

    # Generate the stubs under the directory Sphinx is actually reading
    # (app.srcdir), so the "autogenerated/fluxion" toctree in
    # python/complete_api.rst always resolves -- whether the build is
    # in-tree, out-of-tree, or on Read the Docs.  The tree is gitignored.
    output_path = os.path.join(app.srcdir, "python", "autogenerated")
    # Point apidoc at the fluxion package itself (not its parent) so the
    # generated modules are named `fluxion.*` rather than `python.fluxion.*`.
    fluxion_dir = os.path.join(py_bindings_dir, "fluxion")
    main(["-e", "-f", "-M", "-T", "-o", output_path, fluxion_dir])
...
def setup(app):
    ...
    # Generate Python API stubs before building
...
# ReadTheDocs runs sphinx without first building flux-sched, so the compiled
# CFFI module in `_fluxion` will not exist, causing import errors when autodoc
# imports fluxion.reapi.cli.  The `flux` package (from flux-core) is likewise
# not installed in the docs build environment.  Mock these imports to prevent
# import errors.
autodoc_mock_imports = ["_fluxion._reapi_cli", "flux"]
