                                            ...
                                            bool rsv)
{
    ...
    if (fetcher.exclusive) {
        ...
        if ((span = planner_add_span (plans, at, dur, static_cast<const uint64_t> (g[v].size)))
            == -1) {
            m_err_msg += __FUNCTION__;
    ...
    for (i = 0; i < json_array_size (edges); i++) {
        ...
        if ((added_vtcs.count (source) == 1) || (added_vtcs.count (target) == 1)) {
            ...
            json_object_foreach (name, key, value) {
                g[e].name[std::string (key)] = std::string (json_string_value (value));
                g[e].idata.member_of[std::string (key)] = std::string (json_string_value (value));
            }
