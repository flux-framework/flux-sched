...
- A resource type that occurs more than once among non-slot siblings
  ...
  represented and would be silently miscounted.  Sibling `slot`
...
A `slot` is itself an exclusive allocation container: an `exclusive` field
written on the slot element is ignored and does not change what its children
inherit. To share a resource under a slot, set `exclusive: false` on that
resource (or on a non-slot ancestor of it beneath the slot).
...
- Non-exclusive resources without a unit (non-pooled) are valid only as
  *waypoints*: interior path vertices that define a containment
  constraint — e.g., a shareable `node` or `socket` whose descendants are
  what the job allocates must be found within it. Nothing is drawn from a
  waypoint: it appears in the allocation at its full size with the shared
  marker and remains shareable with other jobs. Within one job, however,
  each slot packs whole vertices, so one waypoint vertex backs at most
  one slot per job regardless of its size. A non-pooled **leaf**
  request whose effective exclusivity is `false` (explicit or inherited
  from an ancestor) is rejected at match time: with neither a capacity
  pool nor an exclusive reservation, nothing would account for the share.
  The rejection is reported in the match error message, e.g.
  `resolve_request: non-pooled resource type 'gpu' cannot be allocated
