                              ...
                              const std::map<resource_type_t, int64_t> &dfu)
{
    ...
    if (opt_subtree_plan && opt_subtree_plan.get () && !dfu.empty ()) {
        int64_t span = -1;
...
int dfu_impl_t::upd_by_outedges (subsystem_t subsystem, jobmeta_t jobmeta, vtx_t u, edg_t e)
{
    ...
    if (opt_subplan && opt_subplan.get ()) {
                              ...
                              bool &stop)
{
    ...
    if (!opt_subtree_plan || !opt_subtree_plan.get ())
        goto done;
...
int dfu_impl_t::clear_vertex (vtx_t vtx, modify_data_t &mod_data)
{
    ...
    if (opt_multi_plans && opt_multi_plans.get ()) {
