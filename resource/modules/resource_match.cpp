                                 ...
                                 json_t *jgf)
{
    ...
    if (db.metadata.roots.find (containment_sub) == db.metadata.roots.end ()) {
        auto jgf_reader = std::dynamic_pointer_cast<resource_reader_jgf_t> (ctx->reader);
        if (!jgf_reader) {
            errno = EINVAL;
            rc = -1;
            flux_log_error (ctx->h, "%s: JGF reader required", __FUNCTION__);
            goto done;
        }
        if ((rc = jgf_reader->set_node_properties (properties)) < 0) {
