...
extern "C" planner_t *planner_copy (planner_t *p)
{
    ...
    try {
        ...
        // type.  An unknown exception cannot be classified, so report
        // ENOMEM like the handlers above rather than guessing.
        errno = ENOMEM;
    }
...
extern "C" int64_t planner_span_next (planner_t *ctx)
{
    ...
    // planner_span_first sets it, after the previous call ran off the end,
    // and after a copy, an assignment, or an erase.  Incrementing an end
