...
extern "C" int planner_multi_assign (planner_multi_t *lhs, const planner_multi_t *rhs)
{
    if (!lhs || !rhs) {
        errno = EINVAL;
        return -1;
                                           ...
                                           size_t len)
{
    ...
    try {
        ...
        // reserve () throws length_error, not bad_alloc, when len exceeds
        // max_size, and no exception may cross this boundary regardless.
        ctx->plan_multi->get_span_lookup ().erase (mspan);
        errno = EINVAL;
        return -1;
    }
    ...
    for (i = 0; i < len; ++i) {
        ...
        if (!ctx->plan_multi->planner_at (resource_types[i])) {
            try {
                ...
                // Backstop: no exception may cross this boundary,
                // whatever its type.
                errno = ENOMEM;
                goto done;
            }
