                                ...
                                const char **resource_types, size_t len)
{
    ...
    try {
        ctx = new planner_multi_t (base_time, duration, resource_totals,
                                   resource_types, len);
...
extern "C" void planner_multi_assign (planner_multi_t *lhs,
                                      planner_multi_t *rhs)
{
...
extern "C" const char *planner_multi_resource_type_at (planner_multi_t *ctx,
                                                       unsigned int i)
{
    if (!ctx) {
        errno = EINVAL;
                       ...
                       const uint64_t *resource_requests, size_t len)
{
    ...
    if ((t = planner_avail_time_first (ctx->plan_multi->get_planner_at (
                                            static_cast<size_t> (0)), on_or_after,
    ...
    } while (unmet && (t = planner_avail_time_next (
                   ...
                   const uint64_t *resource_requests, size_t len)
{
    ...
    for (i = 0; i < ctx->plan_multi->get_planners_size (); ++i) {
        rc = planner_avail_during (ctx->plan_multi->get_planner_at (i), at, duration,
                                   resource_requests[i]);
        if (rc == -1)
            break;
    }
                   ...
                   uint64_t duration, int64_t *resource_counts, size_t len)
{
    ...
    if (!ctx || !resource_counts
        || ctx->plan_multi->get_planners_size () < 1
        || ctx->plan_multi->get_planners_size () != len) {
        errno = EINVAL;
        return -1;
    }
                                          ...
                                          bool &removed)
{
    ...
    for (i = 0; i < len; ++i) {
        ...
        if ( (rc = planner_reduce_span (
                        ctx->plan_multi->get_planner_at (resource_types[i]),
            ...
            if (planner_is_active_span (ctx->plan_multi->get_planner_at (resource_types[i]),
                                        span_it->second.at (mspan_idx))) {
