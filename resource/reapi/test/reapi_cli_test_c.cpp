...
// reapi_cli_get_err_msg () returns a strdup'd buffer on every path.
           ...
           "[initialize-match-satisfy-cancel C]")
{
    ...
    // Install the diagnostic before the assertion so a failure is annotated.
...
TEST_CASE ("REAPI CLI match functions reject null pointer arguments", "[match-null-guards C]")
{
    ...
    errno = 0;
    CHECK (reapi_cli_match_satisfy (nullptr, jobspec.c_str (), &ov) == -1);
    CHECK (errno == EINVAL);
    errno = 0;
    CHECK (reapi_cli_match_satisfy (ctx, nullptr, &ov) == -1);
    CHECK (errno == EINVAL);
    errno = 0;
    CHECK (reapi_cli_match_satisfy (ctx, jobspec.c_str (), nullptr) == -1);
    CHECK (errno == EINVAL);
...
// The remaining CLI bindings must likewise reject each null required pointer
// with EINVAL before dereferencing it.
TEST_CASE ("REAPI CLI non-match functions reject null pointer arguments", "[null-guards C]")
{
    const std::string options = "{}";
    ...
    int64_t at = 0;
    double ov = 0.0;
    const char *R_out = nullptr;
    resource_status_t status = RESOURCE_UP;

    SECTION ("reapi_cli_info")
    {
        struct {
            char **mode;
            bool *reserved;
            ...
            {nullptr, &reserved, &at, &ov},
            {&mode, nullptr, &at, &ov},
            {&mode, &reserved, nullptr, &ov},
            {&mode, &reserved, &at, nullptr},
        };
        for (auto &a : args) {
            errno = 0;
            CHECK (reapi_cli_info (ctx, 1, a.mode, a.reserved, a.at, a.ov) == -1);
            CHECK (errno == EINVAL);
        }
    }

    SECTION ("reapi_cli_update_allocate")
    {
        struct {
            const char *R;
            int64_t *at;
        ...
        for (auto &a : args) {
            errno = 0;
            CHECK (reapi_cli_update_allocate (ctx, 1, a.R, a.at, a.ov, a.R_out) == -1);
            CHECK (errno == EINVAL);
        }
    }

    SECTION ("reapi_cli_partial_cancel")
    {
        errno = 0;
        CHECK (reapi_cli_partial_cancel (ctx, 1, nullptr, false, &full_removal) == -1);
        CHECK (errno == EINVAL);

        errno = 0;
        CHECK (reapi_cli_partial_cancel (ctx, 1, "{}", false, nullptr) == -1);
        CHECK (errno == EINVAL);
    }
    ...
    {
        errno = 0;
        CHECK (reapi_cli_set_rank_status (ctx, nullptr, RESOURCE_UP) == -1);
        CHECK (errno == EINVAL);

        errno = 0;
        CHECK (reapi_cli_get_rank_status (ctx, nullptr, &status) == -1);
        CHECK (errno == EINVAL);

        errno = 0;
        CHECK (reapi_cli_get_rank_status (ctx, "0", nullptr) == -1);
        CHECK (errno == EINVAL);
    }

    SECTION ("stat")
    {
        // Any one of the seven outputs being null must fail with EINVAL.
        int64_t V, E, J;
        double load, min, max, avg;
        for (size_t i = 0; i < 7; i++) {
            int64_t *ints[3] = {&V, &E, &J};
            double *dbls[4] = {&load, &min, &max, &avg};
            if (i < 3)
                ints[i] = nullptr;
            else
                dbls[i - 3] = nullptr;
            INFO ("null stat output " << i);
            errno = 0;
            CHECK (
                reapi_cli_stat (ctx, ints[0], ints[1], ints[2], dbls[0], dbls[1], dbls[2], dbls[3])
                == -1);
            CHECK (errno == EINVAL);
        }
    }
...
TEST_CASE ("REAPI CLI repeated and failed initialization", "[reinitialize C]")
{
    ...
    // Re-initialization must replace the prior resource_query_t.
    REQUIRE (reapi_cli_initialize (ctx, rgraph.c_str (), options.c_str ()) == 0);
...
TEST_CASE ("REAPI CLI satisfiability return codes", "[satisfy C]")
{
    // 0 satisfiable, 1 unsatisfiable, -1 error.  Only errno separates 1 from
    // -1, so an allocator that clobbered it inside free (R) would make the
    ...
    // test011.yaml requests a four-digit count, far beyond what
    // tiny.graphml holds, so the traverser reports the request as
    // unsatisfiable rather than merely unschedulable right now.
    std::stringstream ubuffer;
    std::ifstream unsatfile (std::string (test_srcdir)
                             + "/data/resource/jobspecs/basics/test011.yaml");
    REQUIRE (unsatfile.is_open ());
    ubuffer << unsatfile.rdbuf ();
    std::string unsat_jobspec = ubuffer.str ();
    ...
    errno = 0;
    int rc = reapi_cli_match_satisfy (ctx, jobspec.c_str (), &ov);
    INFO (take_err_msg (ctx));
    CHECK (rc == 0);

    errno = 0;
    rc = reapi_cli_match_satisfy (ctx, unsat_jobspec.c_str (), &ov);
    int saved_errno = errno;
    INFO (take_err_msg (ctx));
    CHECK (rc == 1);
    CHECK (saved_errno == ENODEV);

    // A malformed jobspec is an error, and must never look unsatisfiable.
    errno = 0;
    rc = reapi_cli_match_satisfy (ctx, "not a jobspec", &ov);
    saved_errno = errno;
    INFO (take_err_msg (ctx));
    CHECK (rc == -1);
    CHECK (saved_errno != ENODEV);
