...
{
    // 0 satisfiable, 1 unsatisfiable, -1 error.  Unsatisfiable is
    // distinguished from an error only by errno, so assert on it directly:
    // an allocator that clobbered errno inside reapi_cli_match_satisfy's
    // free (R) would turn a definitive verdict into an unclassifiable one.
    const std::string options = "{}";
    std::stringstream gbuffer, jbuffer;
    const char *test_srcdir = std::getenv ("SHARNESS_TEST_SRCDIR");
    REQUIRE (test_srcdir);
    ...
    errno = 0;
    INFO (take_err_msg (ctx));
    CHECK (reapi_cli_match_satisfy (ctx, unsat_jobspec.c_str (), &ov) == 1);
    CHECK (errno == ENODEV);

    // A malformed jobspec is an error, and must never look unsatisfiable.
    errno = 0;
    CHECK (reapi_cli_match_satisfy (ctx, "not a jobspec", &ov) == -1);
    CHECK (errno != ENODEV);
