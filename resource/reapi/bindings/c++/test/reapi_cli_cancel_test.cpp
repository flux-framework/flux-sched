...
static int test_cancel_with_r ()
{
    ...
    try {
        ...
        ok (0, "FAILED: couldn't create resource_query_t");
        return 0;
    }
...
static int test_cancel_partial ()
{
    ...
    if (!r_obj) {
        ok (1, "# SKIP: couldn't parse R as JSON");
        delete rq;
        return 0;
    }
    ...
    if (!r_lite || !json_is_array (r_lite)) {
        ok (0, "R missing execution.R_lite array");
        json_decref (r_obj);
        delete rq;
        return 0;
    }
    ...
    if (!partial_r) {
        ok (1, "# SKIP: couldn't serialize partial R");
        delete rq;
        return 0;
    }
