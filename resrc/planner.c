...
static int scheduled_point_insert (scheduled_point_t *data, rb_root_t *root)
{
    rb_node_t **nn = &(root->rb_node);
    rb_node_t *parent = NULL;
    while (*nn) {
        scheduled_point_t *th = container_of(*nn, scheduled_point_t, point_rb);
        int64_t result = data->at - th->at;
        parent = *nn;
        if (result < 0)
            nn = &((*nn)->rb_left);
        else if (result > 0)
    ...
    rb_link_node(&data->point_rb, parent, nn);
    rb_insert_color(&data->point_rb, root);
    return 0;
}
...
static int scheduled_point_remove (scheduled_point_t *data, struct rb_root *root)
{
    ...
    if (n) {
        ...
        /* This only removes the node from this BST
         * and does not free the memory allocated to the node
         */
        rc = 0;
    }
    return rc;
...
static void scheduled_resrc_insert (scheduled_point_t *data_p, rb_root_t *root)
{
    rb_node_t **link = &(root->rb_node);
    scheduled_point_t *th = NULL;
    rb_node_t *rbp = NULL;
    while (*link) {
        rbp = *link;
        th = rb_entry(rbp, scheduled_point_t, resrc_rb);
        if (th->__subtree_min > data_p->at)
            th->__subtree_min = data_p->at;
        int64_t result = 0;
        result = veccmp (data_p->remaining_resrcs,
                     th->remaining_resrcs, MAX_RESRC_DIM);

        if ( result < 0)
            link = &(th->resrc_rb.rb_left);
        else
            link = &(th->resrc_rb.rb_right);
    }
    data_p->__subtree_min = data_p->at;
    data_p->inserted_to_resrc = 1;
    rb_link_node(&(data_p->resrc_rb), rbp, link);
    rb_insert_augmented(&data_p->resrc_rb, root, &scheduled_resrc_aug_cb);
}

static void scheduled_resrc_remove (scheduled_point_t *point, rb_root_t *root)
{
    rb_erase_augmented (&point->resrc_rb, root, &scheduled_resrc_aug_cb);
    point->inserted_to_resrc = 0;
}
...
static inline int64_t rbranch_mintm (rb_node_t *node)
{
    ...
    scheduled_point_t *point = rb_entry(node, scheduled_point_t, resrc_rb);
    return  (point->at < mn)? point->at : mn;
}
                                      ...
                                      int64_t mintm)
{
    ...
    point = rb_entry(anchor, scheduled_point_t, resrc_rb);
    if (point->at == mintm)
        return point;

    rb_node_t *node = anchor->rb_right;
    while (node) {
        point = rb_entry(node, scheduled_point_t, resrc_rb);
        if (point->at == mintm)
            return point;
                          ...
                          rb_node_t **anchor_p)
{
    ...
    while (node) {
        point = rb_entry(node, scheduled_point_t, resrc_rb);
        int64_t result = 0;
        result = veccmp (rv, point->remaining_resrcs, MAX_RESRC_DIM);
        if (result <= 0) {
...
static inline bool sat_Is (reservation_t *r1, int64_t s, int64_t l,
                       int64_t *resrc_vector, size_t vector_dim)
{
    bool rc = true;
    if (s < LAST(r1) && l > START(r1)) {
        scheduled_point_t *sp = r1->start_p;
        rc = veccmp ((int64_t *)resrc_vector, sp->remaining_resrcs,
                 vector_dim) <= 0;
    }
    return rc;
}
...
static inline int64_t avail_time_internal (planner_t *ctx, req_t *req)
{
    ...
    if (veccmp ((int64_t *)req->resrc_vector,
                 ctx->total_resrc_vector, req->vector_dim) > 0) {
        errno = ERANGE;
    ...
    while (!sat && (p = scheduled_resrc_mintm ((int64_t *)req->resrc_vector,r))) {
        ...
        while ((d_chk = rb_entry(n, scheduled_point_t, point_rb))) {
            if (OUT_OF_RANGE(d_chk->at, p->at, req->duration))
                ...
                int64_t result = 0;
                if ( (result = veccmp ((int64_t *)req->resrc_vector,
                          d_chk->remaining_resrcs, req->vector_dim)) > 0) {
...
static inline int avail_resources_at_internal (planner_t *ctx,
                      int64_t starttime, int64_t lasttime, int64_t *rv, int vd)
{
    ...
    /* Go through each and all of the existing reservations with which this request
     * intersects and check if all of their scheduled points satisfy the request.
     */
    for (i = reservation_iter_first (rr, s, l); i;
         i = reservation_iter_next (i, s, l)) {
        if (!sat_Is (i, s, l, rv, vd))
            goto done;
    }
    avail = 0;
...
{
    return avail_resources_at_internal (ctx, starttime,
               starttime+req->duration, (int64_t *)req->resrc_vector,
               req->vector_dim);
}
...
int planner_add_reservation (planner_t *ctx, reservation_t *rsv, int validate)
{
    ...
    if (!rsv || !ctx) {
        ...
        if (avail_resources_at_internal (ctx, rsv->start,
                rsv->last, rsv->reserved_resrcs, rsv->resrc_dim) == -1)
            goto done2;
    }
