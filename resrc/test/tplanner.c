...
void test_2xr0_4_10p_2D_unmet ()
{
    ...
    req2 = pt_req_new (2, 5, 1, 20, 100, 1000, 10000);
    starttime = planner_avail_time_first (ctx, req2);
    ok (starttime == 2, "find first availability for {<1,20,100,1000,10000>, 2}");
...
void test_misc ()
{
    ...
    free (plan);
    pt_req_free (req);
    rc = planner_add_reservation (ctx, rsv2, 1);
    char *str = planner_reservation_to_string (ctx, rsv2);
    ...
    ok ((i == 7), "planner_revervation iterator works");
    for (i=0; i < 5; ++i)
        planner_reservation_destroy (ctx, &(ra[i]));
    free (ra);
    ra = NULL;
